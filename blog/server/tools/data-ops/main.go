...
import (
	...
	"github.com/hofstadter-io/examples/blog/server/tools/data-ops/commands"
...
func read_config() {
	...
	viper.AddConfigPath(".")
	// viper.SetConfigName("server-tool-data")
	// viper.AddConfigPath("$HOME/.server-tool-data")
	viper.MergeInConfig()

	// Hackery because viper only takes the first config file found... not merging, wtf does merge config mean then anyway
	f, err := os.Open("server-tool-data.yml")
	if err != nil {
		f = nil
		f2, err2 := os.Open("server-tool-data.yaml")
		if err2 != nil {
			f = nil
		} else {
			f = f2
