...
import (
	...
	"github.com/hofstadter-io/examples/blog/server/tools/db-ops/commands"
...
func read_config() {
	...
	viper.AddConfigPath(".")
	// viper.SetConfigName("serverToolDB")
	// viper.AddConfigPath("$HOME/.serverToolDB")
	viper.MergeInConfig()

	// Hackery because viper only takes the first config file found... not merging, wtf does merge config mean then anyway
	f, err := os.Open("serverToolDB.yml")
	if err != nil {
		f = nil
		f2, err2 := os.Open("serverToolDB.yaml")
		if err2 != nil {
			f = nil
		} else {
			f = f2
