...
import (
	...
	"github.com/hofstadter-io/examples/blog/server/client/server-cli/commands"
...
func read_config() {
	...
	viper.AddConfigPath(".")
	// viper.SetConfigName("server-cli")
	// viper.AddConfigPath("$HOME/.server-cli")
	viper.MergeInConfig()

	// Hackery because viper only takes the first config file found... not merging, wtf does merge config mean then anyway
	f, err := os.Open("server-cli.yml")
	if err != nil {
		f = nil
		f2, err2 := os.Open("server-cli.yaml")
		if err2 != nil {
			f = nil
		} else {
			f = f2
